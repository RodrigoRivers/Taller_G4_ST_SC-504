...
(
    ...
    CONSTRAINT fk_factura_cliente FOREIGN KEY (cliente_id) REFERENCES Clientes_t(cliente_id),
...
CREATE OR REPLACE PROCEDURE VERIFICAR_PLACA_EXISTENTE (
  N_VEHICULO_PLACA IN VARCHAR2,
  N_RESULTADO OUT NUMBER
)
AS
BEGIN
  SELECT COUNT(*) INTO N_RESULTADO FROM VEHICULOS WHERE VEHICULO_PLACA = N_VEHICULO_PLACA;
END VERIFICAR_PLACA_EXISTENTE;
...
CREATE OR REPLACE PROCEDURE VERIFICAR_CITAS_DIA(
  fecha IN VARCHAR2,
  mecanico_id IN NUMBER,
  cantidad OUT NUMBER
) AS
BEGIN
  SELECT COUNT(*)
  INTO cantidad
  FROM Citas
  WHERE cita_fecha = fecha AND mecanico_id = mecanico_id;
END;
...
CREATE OR REPLACE PROCEDURE INSERTARFACTURAS (
N_FACTURA_ID IN NUMBER,
N_CLIENTE_ID IN NUMBER,
N_REPUESTO_ID IN NUMBER,
N_FACTURA_MONTO IN NUMBER,
N_FACTURA_FECHA IN VARCHAR2
)
AS
BEGIN
INSERT INTO FACTURAS_t (
FACTURA_ID,
CLIENTE_ID,
REPUESTO_ID,
FACTURA_MONTO,
...
END INSERTARFACTURAS;

------ Eliminar ------

CREATE OR REPLACE PROCEDURE ELIMINARFACTURAS (
N_FACTURA_ID IN NUMBER
)
AS
BEGIN
DELETE FROM FACTURAS WHERE FACTURA_ID = N_FACTURA_ID;
DBMS_OUTPUT.PUT_LINE('Registro eliminado exitosamente');
END ELIMINARFACTURAS;

------ Actualizar ------

CREATE OR REPLACE PROCEDURE ACTUALIZARFACTURAS (
N_FACTURA_ID IN NUMBER,
N_CLIENTE_ID IN NUMBER,
N_REPUESTO_ID IN NUMBER,
N_FACTURA_MONTO IN NUMBER,
...
CLIENTE_ID = N_CLIENTE_ID,
REPUESTO_ID = N_REPUESTO_ID,
FACTURA_MONTO = N_FACTURA_MONTO,
FACTURA_FECHA = N_FACTURA_FECHA
WHERE FACTURA_ID = N_FACTURA_ID;
END ACTUALIZARFACTURAS;
